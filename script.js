...
function buildWheel() {
  ...
  wheel.querySelectorAll('.segment-label').forEach((label) => label.remove());

  const wheelSize = wheel.clientWidth || Math.min(window.innerWidth * 0.74, 430);
  const radius = Math.max(56, wheelSize * 0.24);
...
function spinWheel() {
  ...
  const canPlayAudio = ensureAudioContext();
  spinning = true;
  spinBtn.disabled = true;
  statusEl.textContent = 'Girando... preparando a surpresa! 💫';
  if (canPlayAudio) playSpinSound(5600);
